...
```{r,echo=TRUE}
fit1 <- lm(log(SalePrice) ~ `Garage Area`+`1st Flr SF`+`Gr Liv Area`+`Full Bath`+`Year Built`,
           ames_raw_2008)
```
...
neighborhood_count <- count(group_by(ames_raw, Neighborhood), Neighborhood)
names(neighborhood_count)[2] <- "freq"

# Plot using ggplot
ggplot(data = neighborhood_count, aes(x = Neighborhood, y = freq)) +
...
```{r,echo=TRUE}
summary(fit1)
fit2 <- lm(log(SalePrice) ~ `Garage Area`+`1st Flr SF`+`Gr Liv Area`+`Full Bath`+`Year Built`+`Year Remod/Add`,ames_raw_2008)
summary（fit2)
```
...
```         
It fits better after feature enginnering
```
